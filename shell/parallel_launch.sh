...
# Parallel execution
parallel -j $max_jobs "python -u ./scripts/spikes_model.py --alpha ${alpha} --teacher ${teacher} --loss ${loss} --N_walkers ${N_walkers} --model ${model} --mode ${mode} --d {1} --lr {2} --student {3} --variation {4} > ./logs/run_repetita_iuvant_d{1}_lr{2}_student{3}_variation{4}.log 2>&1" ::: "${d_values[@]}" ::: "${lr_values[@]}" ::: "${students[@]}" ::: "${variations[@]}"
